...
static func draw_scene():
	...
	for file : String in files:
		...
		if globals.savegame_temp != null:
			globals.write(save.player.name + (" & " + save.companion.name if save.companion != null else ""))
			globals.set_cursor_x(33)
			globals.write(globals.savegame_temp.format_date())
			globals.set_cursor_x(49)
			globals.write(globals.savegame_temp.current_area)
