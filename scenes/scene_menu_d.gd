...
static func draw_scene():
	...
	globals.write("These are the game settings, you can modify them freely here.", "Yellow")
	...
	globals.write_selectable(func():
		if globals.settings["max_window_scale"] <= 1: globals.settings["max_window_scale"] = 2
		elif globals.settings["max_window_scale"] == 2: globals.settings["max_window_scale"] = 3
		elif globals.settings["max_window_scale"] == 3: globals.settings["max_window_scale"] = 4
		elif globals.settings["max_window_scale"] >= 4: globals.settings["max_window_scale"] = 1
		globals.save_file(globals.settings, "Settings", "settings")
	)
	globals.write("Max Window Scale")
	globals.set_cursor_x(33)
	globals.write("x"+ str(int(globals.settings["max_window_scale"])))
	globals.set_cursor_x(0)
	globals.modify_cursor_y(1)
	globals.write("-".repeat(80))
	globals.set_cursor_x(1)
	globals.modify_cursor_y(1)
	globals.write("Visuals")
	globals.set_cursor_x(0)
	globals.modify_cursor_y(1)
	globals.write("-".repeat(80))
	globals.set_cursor_x(1)
	...
	globals.modify_cursor_y(1)
	globals.write_selectable(func():
		if globals.settings["effects_volume"] >= 10:
			globals.settings["effects_volume"] -= 10
	...
	globals.write_selectable(func():
		if globals.settings["effects_volume"] < 100:
			globals.settings["effects_volume"] += 10
			globals.fade_bus_to("Effects", globals.settings["effects_volume"], 0.1)
			globals.save_file(globals.settings, "Settings", "settings")
	)
	globals.write("+10%")
	globals.set_cursor_x(33)
	globals.write(str(int(globals.settings["effects_volume"])) + "%")
	globals.set_cursor_x(0)
	globals.modify_cursor_y(1)
	globals.write("-".repeat(80))
	globals.set_cursor_x(1)
	...
	globals.modify_cursor_y(1)
	globals.write_selectable(func():
		if globals.settings["music_volume"] >= 10:
			globals.settings["music_volume"] -= 10
			globals.fade_bus_to("Music", globals.settings["music_volume"], 0.5)
			globals.save_file(globals.settings, "Settings", "settings")
	)
	globals.write("-10%")
	globals.modify_cursor_x(1)
	globals.write_selectable(func():
		if globals.settings["music_volume"] < 100:
			globals.settings["music_volume"] += 10
			globals.fade_bus_to("Music", globals.settings["music_volume"], 0.5)
			globals.save_file(globals.settings, "Settings", "settings")
	)
	globals.write("+10%")
	globals.set_cursor_x(33)
	globals.write(str(int(globals.settings["music_volume"])) + "%")
